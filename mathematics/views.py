...
def mat(request):
    ...
    mat=MatClass.objects.all()
    curdate=date.today()
    curtime=datetime.now().time()
    return render(request,'mathematics.html',{'mat':mat,'curdate':curdate, 'curtime':curtime})
...
def book3(request):
    ...
    for boo in b:
        if(int(boo.classno)==int(classno) and str(boo.username) == str(name) and str(boo.sub)=="maths"):
    ...
    if int(mindose)==1:
        if dose1:
            bo=bookings(username=name,classno=classno,sub="maths",date=date,time=time)
            bo.save()
        else:
            return HttpResponse("Vaccinate 1st Dose to book class")
    else:
        if dose2:
            bo=bookings(username=name,classno=classno,sub="physics",date=date,time=time)
            bo.save()
        else:
            return render(request,"d1.html")

        
    message_to_broadcast = ("Your slot on "+str(date)+ " class no "+ str(classno)+" of mathematics has been booked")
