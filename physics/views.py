...
from .models import PhysicsClass
...
def phy(request):
    ...
    phy=PhysicsClass.objects.all()
    curdate=date.today()
    curtime=datetime.now().time()
    return render(request,'physics.html',{'phy':phy,'curdate':curdate, 'curtime':curtime})
...
def book1(request):
    ...
    for boo in b:
        if(int(boo.classno)==int(classno) and str(boo.username) == str(name) and str(boo.sub)=="physics"):
    ...
    message_to_broadcast = ("Your physics class: "+str(classno)+ " on "+str(date) +" has been booked.")
